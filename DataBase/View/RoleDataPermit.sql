...
select D.RoleId,
       ...
       case when D.Permission = 0 then '只读' when D.Permission = 1 then '读写' else null end as Permit
...
select A.RoleId,
       ...
       4 as Type,
       O.FullName as Model,
       case when A.Permission = 0 then '只读' when A.Permission = 1 then '读写' else null end as Permit
