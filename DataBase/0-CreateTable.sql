...
DROP TABLE SYS_RolePerm_Data
GO
IF EXISTS (SELECT * FROM sysobjects WHERE id = OBJECT_ID(N'SYS_RolePerm_Action') AND OBJECTPROPERTY(id, N'ISUSERTABLE') = 1)
DROP TABLE SYS_RolePerm_Action
GO

IF EXISTS (SELECT * FROM sysobjects WHERE id = OBJECT_ID(N'SYS_Role_Title') AND OBJECTPROPERTY(id, N'ISUSERTABLE') = 1)
DROP TABLE SYS_Role_Title
GO
IF EXISTS (SELECT * FROM sysobjects WHERE id = OBJECT_ID(N'SYS_Role_UserGroup') AND OBJECTPROPERTY(id, N'ISUSERTABLE') = 1)
DROP TABLE SYS_Role_UserGroup
GO
IF EXISTS (SELECT * FROM sysobjects WHERE id = OBJECT_ID(N'SYS_Role_User') AND OBJECTPROPERTY(id, N'ISUSERTABLE') = 1)
...
IF EXISTS (SELECT * FROM sysobjects WHERE id = OBJECT_ID(N'SYS_OrgMerger') AND OBJECTPROPERTY(id, N'ISUSERTABLE') = 1)
DROP TABLE SYS_OrgMerger
GO
...
[PositionId]       UNIQUEIDENTIFIER,                                                                                                       --岗位ID，字典
[Validity]         BIT DEFAULT 1 NOT NULL,                                                                                                 --是否有效：0、无效；1、有效
[CreatorUserId]    UNIQUEIDENTIFIER FOREIGN KEY REFERENCES SYS_User(ID) DEFAULT '00000000-0000-0000-0000-000000000000' NOT NULL,           --创建人ID
[CreateTime]       DATETIME DEFAULT GETDATE() NOT NULL                                                                                     --创建时间
)
...
CREATE TABLE SYS_OrgMerger(
[ID]               UNIQUEIDENTIFIER CONSTRAINT IX_SYS_OrgMerger PRIMARY KEY DEFAULT NEWSEQUENTIALID(),
[SN]               BIGINT IDENTITY(1,1),                                                                                                   --自增序列
[OrgId]            UNIQUEIDENTIFIER FOREIGN KEY REFERENCES SYS_Organization(ID) ON DELETE CASCADE NOT NULL,                                --组织机构ID
[MergerOrgId]      UNIQUEIDENTIFIER FOREIGN KEY REFERENCES SYS_Organization(ID) NOT NULL,                                                  --被并组织机构节点ID（只允许同类型合并）
[CreatorUserId]    UNIQUEIDENTIFIER FOREIGN KEY REFERENCES SYS_User(ID) DEFAULT '00000000-0000-0000-0000-000000000000' NOT NULL,           --创建人ID
...
[ParentId]         UNIQUEIDENTIFIER,                                                                                                       --父模块ID
...
[ID]               UNIQUEIDENTIFIER CONSTRAINT IX_SYS_Role_User PRIMARY KEY DEFAULT NEWSEQUENTIALID(),
[SN]               BIGINT IDENTITY(1,1),                                                                                                   --自增序列
[RoleId]           UNIQUEIDENTIFIER FOREIGN KEY REFERENCES SYS_Role(ID) ON DELETE CASCADE NOT NULL,                                        --角色ID
[UserId]           UNIQUEIDENTIFIER FOREIGN KEY REFERENCES SYS_User(ID) NOT NULL,                                                          --用户ID
[CreatorUserId]    UNIQUEIDENTIFIER FOREIGN KEY REFERENCES SYS_User(ID) DEFAULT '00000000-0000-0000-0000-000000000000' NOT NULL,           --创建人ID
[CreateTime]       DATETIME DEFAULT GETDATE() NOT NULL                                                                                     --创建时间
)
GO
...
[ID]               UNIQUEIDENTIFIER CONSTRAINT IX_SYS_Role_UserGroup PRIMARY KEY DEFAULT NEWSEQUENTIALID(),
[SN]               BIGINT IDENTITY(1,1),                                                                                                   --自增序列
[RoleId]           UNIQUEIDENTIFIER FOREIGN KEY REFERENCES SYS_Role(ID) ON DELETE CASCADE NOT NULL,                                        --角色ID
[GroupId]          UNIQUEIDENTIFIER FOREIGN KEY REFERENCES SYS_UserGroup(ID) ON DELETE CASCADE NOT NULL,                                   --用户组ID
[CreatorUserId]    UNIQUEIDENTIFIER FOREIGN KEY REFERENCES SYS_User(ID) DEFAULT '00000000-0000-0000-0000-000000000000' NOT NULL,           --创建人ID
...
CREATE TABLE SYS_Role_Title(
[ID]               UNIQUEIDENTIFIER CONSTRAINT IX_SYS_Role_Title PRIMARY KEY DEFAULT NEWSEQUENTIALID(),
[SN]               BIGINT IDENTITY(1,1),                                                                                                   --自增序列
[RoleId]           UNIQUEIDENTIFIER FOREIGN KEY REFERENCES SYS_Role(ID) ON DELETE CASCADE NOT NULL,                                        --角色ID
[OrgId]            UNIQUEIDENTIFIER FOREIGN KEY REFERENCES SYS_Organization(ID) NOT NULL,                                                  --组织机构（职位）ID
[CreatorUserId]    UNIQUEIDENTIFIER FOREIGN KEY REFERENCES SYS_User(ID) DEFAULT '00000000-0000-0000-0000-000000000000' NOT NULL,           --创建人ID
[CreateTime]       DATETIME DEFAULT GETDATE() NOT NULL                                                                                     --创建时间
)
GO
...
CREATE TABLE SYS_RolePerm_Action(
...
CREATE TABLE SYS_RolePerm_Data(
[ID]               UNIQUEIDENTIFIER CONSTRAINT IX_SYS_RolePerm_Data_Rel PRIMARY KEY DEFAULT NEWSEQUENTIALID(),
[SN]               BIGINT IDENTITY(1,1),                                                                                                   --自增序列
[RoleId]           UNIQUEIDENTIFIER FOREIGN KEY REFERENCES SYS_Role(ID) ON DELETE CASCADE NOT NULL,                                        --角色ID
[ModuleId]         UNIQUEIDENTIFIER FOREIGN KEY REFERENCES SYS_Module(ID) ON DELETE CASCADE NOT NULL,                                      --模块注册ID
[Mode]             INT DEFAULT 0 NOT NULL,                                                                                                 --数据授权范围：0、无归属；1、仅本人；2、仅本部门；3、本部门所有；4、本机构所有；5、本根域所有
...
CREATE TABLE SYS_RolePerm_DataAbs(
[ID]               UNIQUEIDENTIFIER CONSTRAINT IX_SYS_RolePerm_Data_Abs PRIMARY KEY DEFAULT NEWSEQUENTIALID(),
[SN]               BIGINT IDENTITY(1,1),                                                                                                   --自增序列
[RoleId]           UNIQUEIDENTIFIER FOREIGN KEY REFERENCES SYS_Role(ID) ON DELETE CASCADE NOT NULL,                                        --角色ID
[ModuleId]         UNIQUEIDENTIFIER FOREIGN KEY REFERENCES SYS_Module(ID) ON DELETE CASCADE NOT NULL,                                      --模块注册ID
...
CREATE TRIGGER SYS_Module_Delete ON SYS_Module AFTER DELETE AS

BEGIN
SET NOCOUNT ON

DELETE M from Deleted D
  join SYS_Module M on M.ParentId = D.ID

END
GO

/*****触发器：增加合并记录后检查已合并记录，如有该节点已经被合并，自动合并到现节点*****/

CREATE TRIGGER SYS_OrgMerger_Insert ON SYS_OrgMerger AFTER INSERT AS

BEGIN
SET NOCOUNT ON

UPDATE O set Validity = 0 from Inserted I
  join SYS_Organization O on O.ID = I.MergerOrgId

UPDATE OM set OrgId = I.OrgId from Inserted I
  join SYS_OrgMerger OM on OM.OrgId = I.MergerOrgId

END
GO

/*****触发器：删除组织机构后自动更新被合并节点状态为有效*****/

CREATE TRIGGER SYS_OrgMerger_Delete ON SYS_OrgMerger AFTER DELETE AS
