...
namespace Insight.Base.OAuth
{
    ...
    {
        private readonly List<Guid> _RoleList;
        ...
        /// <param name="all">全部角色</param>
        public Authority(Guid uid, Guid? did, bool all = false)
        {
            _RoleList = UserRoles(uid, did, all);
        }
        ...
        public IEnumerable<object> ModuleActions(Guid mid)
        {
            ...
            {
                var allows = context.SYS_Role_Action.Where(a => a.SYS_ModuleAction.ModuleId == mid && _RoleList.Any(id => id == a.RoleId));
        ...
        public bool Identify(Guid aid)
        {
            using (var context = new BaseEntities())
            {
                var actions = context.SYS_Role_Action.Where(a => a.ActionId == aid && _RoleList.Any(id => id == a.RoleId));
        ...
        public IEnumerable<RoleAction> GetUserActions()
        {
            using (var context = new BaseEntities())
            {
                var groups = from id in RoleModules().Select(m => m.GroupId).Distinct()
                             join g in context.SYS_ModuleGroup on id equals g.ID
                             select new RoleAction {ID = g.ID, Index = g.Index, NodeType = 0, Name = g.Name};
                var modules = from id in RoleModules().Select(m => m.ID).Distinct()
                              join m in context.SYS_Module on id equals m.ID
                              select new RoleAction {ID = m.ID, ParentId = m.ModuleGroupId, Index = m.Index, NodeType = 1, Name = m.ApplicationName};
                var actions = from id in RoleModules().Select(m => m.ID).Distinct()
                              join a in context.SYS_ModuleAction on id equals a.ModuleId
                              let allows = context.SYS_Role_Action.Where(p => p.ActionId == a.ID).OrderBy(i => i.Action).FirstOrDefault()
                ...
                return groups.OrderBy(g => g.Index).Union(modules.OrderBy(m => m.Index)).Union(actions.OrderBy(a => a.Index)).ToList();
            }
        }
        ...
        public IEnumerable<RoleData> GetUserDatas()
        {
            using (var context = new BaseEntities())
            {
                var list = from d in context.RoleData.ToList()
        ...
        private IEnumerable<ModuleInfo> RoleModules()
        {
            using (var context = new BaseEntities())
            {
                var list = from r in context.SYS_Role_Action.Where(r => _RoleList.Any(id => id == r.RoleId))
        ...
        private List<Guid> UserRoles(Guid uid, Guid? did, bool all)
        {
            using (var context = new BaseEntities())
            {
