    ...
    public class SqlHelper
    {
        ...
        public static DataTable SqlQuery(SqlCommand cmd)
        {
            using (var conn = new SqlConnection(ConStr))
            {
                ...
                {
                    var msg = Util.Serialize(new {SQL = cmd.CommandText, Exception = ex.Message});
                    new Logger("200101", msg).Write();
                    return null;
                }
            }
        }
        ...
        public static bool SqlExecute(IEnumerable<SqlCommand> cmds)
        {
            using (var conn = new SqlConnection(ConStr))
            {
                ...
                {
                    var msg = Util.Serialize(new { SQL = sql, Exception = ex.Message });
                    new Logger("200104", msg).Write();
                    tran.Rollback();
                    return false;
                }
            }
